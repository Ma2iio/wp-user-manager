...
function wpum_upload_file( $file, $args = array() ) {
	...
	if ( ! in_array( $file['type'], $args['allowed_mime_types'] ) ) {
		...
		echo "<pre>";
		print_r($file);
		$fileType = ".".wp_check_filetype( $file['name'] )['ext']; // substring for find type file
		$fileName = basename( $file['name'], $fileType); // get file name
		$file['name'] = md5($fileName).$fileType; //mixed file name
