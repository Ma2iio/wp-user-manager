...
class WPUM_Form_Register extends WPUM_Form {
	...
	public static function do_registration( $username, $email, $values ) {
		...
		if ( is_wp_error( $do_user ) ) {
			...
			//For custom field
			if( array_key_exists( 'national_id_card' , $values['register'] ) )// For national_id_card's upload
				update_user_meta( $user_id, 'national_id_card', $values['register']['national_id_card']['url']);
			if( array_key_exists( 'bookbank_card' , $values['register'] ) )// For Book Bank Card's upload
				update_user_meta( $user_id, 'bookbank_card', $values['register']['bookbank_card']['url']);
				// esc_url()
